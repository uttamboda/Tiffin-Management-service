...
class OrderServiceTest {
    ...
    void getAllOrders_ReturnsPage() {
        ...
        org.springframework.data.domain.Page<Order> orderPage = new org.springframework.data.domain.PageImpl<>(
                List.of(order1));

        when(shopService.getCurrentShop()).thenReturn(mockShop);
        when(orderRepository.findAllByShop_Id(eq(1L), any(org.springframework.data.domain.PageRequest.class)))
                .thenReturn(orderPage);

        // Act
        org.springframework.data.domain.Page<OrderResponseDTO> result = orderService.getAllOrders(0, 10);

        // Assert
        assertNotNull(result);
        assertEquals(1, result.getContent().size());
        assertEquals(1L, result.getContent().get(0).getId());
        verify(orderRepository, times(1)).findAllByShop_Id(eq(1L),
    ...
    void getOrderById_Success() {
        ...
        when(shopService.getCurrentShop()).thenReturn(mockShop);
        when(orderRepository.findByIdAndShop_Id(orderId, 1L)).thenReturn(Optional.of(order));
        ...
        verify(orderRepository, times(1)).findByIdAndShop_Id(orderId, 1L);
    }
    ...
    void getOrderById_NotFound_ThrowsException() {
        Long orderId = 1L;
        when(shopService.getCurrentShop()).thenReturn(mockShop);
        when(orderRepository.findByIdAndShop_Id(orderId, 1L)).thenReturn(Optional.empty());
